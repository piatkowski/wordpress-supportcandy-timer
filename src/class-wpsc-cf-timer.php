...
if ( ! class_exists( 'WPSC_CF_Timer' ) ) :

	final class WPSC_CF_Timer {
		...
		public static function init() {
			...
			add_action( 'wpsc_change_ticket_status', array( __CLASS__, 'run_timer_on_status_change' ), 10, 4 );
		}

		public static function run_timer_on_status_change( $ticket, $prev, $new, $customer_id ) {

			$cf_slug = 'cust_' . Timer_Support_Candy::TIMER_FIELD_ID;
			if ( ! $cf_slug ) {
				return;
			}

			// Trigger run
			if ( in_array( $new, Timer_Support_Candy::STATUS_TRIGGER ) ) {

				$ticket->{$cf_slug}   = new DateTime();
			...
			// Stop timer
			if ( in_array( $new, Timer_Support_Candy::STATUS_STOP ) ) {
		...
		public static function add_cf_type( $cf_types ) {
			...
			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'Timer', 'supportcandy' ),
				'class' => __CLASS__,
			);
		...
		public static function get_operators( $cf, $filter = array() ) { ?>
            ...
            <div class="item conditional">
                <select class="operator"
                        onchange="wpsc_get_ticket_filter_operands(this, <?php echo esc_attr( $cf->id ); ?>, '<?php echo esc_attr( wp_create_nonce( 'wpsc_get_ticket_filter_operands' ) ); ?>');">
                    <option value=""><?php esc_attr_e( 'Compare As', 'supportcandy' ); ?></option>
                    <option <?php isset( $filter['operator'] ) && selected( $filter['operator'], '=' ); ?>
                            value="="><?php esc_attr_e( 'Equals', 'supportcandy' ); ?></option>
                    <option <?php isset( $filter['operator'] ) && selected( $filter['operator'], '<' ); ?>
                            value="<"><?php esc_attr_e( 'Less than', 'supportcandy' ); ?></option>
                    <option <?php isset( $filter['operator'] ) && selected( $filter['operator'], '<=' ); ?>
                            value="<="><?php esc_attr_e( 'Less than or equals', 'supportcandy' ); ?></option>
                    <option <?php isset( $filter['operator'] ) && selected( $filter['operator'], '>' ); ?>
                            value=">"><?php esc_attr_e( 'Greater than', 'supportcandy' ); ?></option>
                    <option <?php isset( $filter['operator'] ) && selected( $filter['operator'], '>=' ); ?>
                            value=">="><?php esc_attr_e( 'Greater than or equals', 'supportcandy' ); ?></option>
                    <option <?php isset( $filter['operator'] ) && selected( $filter['operator'], 'BETWEEN' ); ?>
                            value="BETWEEN"><?php esc_attr_e( 'Between', 'supportcandy' ); ?></option>
                </select>
            </div>
			<?php
		}
		...
		public static function get_operands( $operator, $cf, $filter = array() ) {
            ...
            <div class="item conditional operand <?php echo ! $is_between ? 'single' : ''; ?>">
                <input type="text" class="operand_val_1 <?php echo esc_attr( $unique_id ); ?>"
                       placeholder="<?php echo $is_between ? esc_attr__( 'From date', 'supportcandy' ) : ''; ?>"
                       value="<?php echo esc_attr( $operand_val_1 ); ?>">
            </div>
			...
			if ( $is_between ) {
                ...
                <div class="item conditional operand">
                    <input type="text" class="operand_val_2 <?php echo esc_attr( $unique_id ); ?>"
                           placeholder="<?php esc_attr_e( 'To date', 'supportcandy' ); ?>"
                           value="<?php echo esc_attr( $operand_val_2 ); ?>">
                </div>
                <script>
		...
		public static function parse_filter( $cf, $compare, $val ) {
			...
			switch ( $compare ) {
				...
				case '<':
					$from = WPSC_Functions::get_utc_date_str( $val . ' 00:00:00' );
					$arr  = array(
						self::get_sql_slug( $cf ) . $compare . '\'' . $from . '\'',
				...
				case '>=':
					...
					$to   = WPSC_Functions::get_utc_date_str( $val . ' 23:59:59' );
					$arr  = array(
						self::get_sql_slug( $cf ) . $compare . '\'' . $to . '\'',
						self::get_sql_slug( $cf ) . ' BETWEEN \'' . $from . '\' AND \'' . $to . '\'',
					);
					$str  = '(' . implode( ' OR ', $arr ) . ')';
					break;
		...
		public static function print_tff( $cf, $tff ) {
			...
			ob_start();
			?>
            <div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>"
                 data-cft="<?php echo esc_attr( self::$slug ); ?>">
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="textfield" data-required="false" class="wpsc-input-group default_value">
                    ...
                    <script>jQuery("#wpsc-default-val-date").flatpickr();</script>
                </div>
			<?php
			endif;
            ...
            <div data-type="date-select" data-required="false" class="wpsc-input-group wpsc-date-range-option"
                 style="display:none;">
                <div class="label-container">
                ...
                <script>
                    jQuery('#from-date-range').flatpickr({disableMobile: true});
		...
		public static function get_edit_custom_field_properties( $cf, $field_class ) {
			...
			if ( in_array( 'extra_info', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="textfield" data-required="false" class="wpsc-input-group extra-info">
                    ...
                    <input name="extra_info" type="text" value="<?php echo esc_attr( $cf->extra_info ); ?>"
                           autocomplete="off"/>
                </div>
			<?php
			endif;
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="textfield" data-required="false" class="wpsc-input-group default_value">
                    ...
                    <div class="wpsc-input-close-group">
                        <input type="text" id="wpsc-default-val-date" name="default_value"
                               value="<?php echo esc_attr( $default_val ); ?>" autocomplete="off"/>
                        <span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
                    </div>
                    <script>
			...
			if ( in_array( 'is_auto_fill', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="single-select" data-required="false" class="wpsc-input-group is_auto_fill">
                    ...
                    <select name="is_auto_fill">
                        <option <?php selected( $cf->is_auto_fill, '0' ); ?>
                                value="0"><?php esc_attr_e( 'No', 'supportcandy' ); ?></option>
                        <option <?php selected( $cf->is_auto_fill, '1' ); ?>
                                value="1"><?php esc_attr_e( 'Yes', 'supportcandy' ); ?></option>
                    </select>
                </div>
			<?php
			endif;
			...
			if ( in_array( 'placeholder_text', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="textfield" data-required="false" class="wpsc-input-group placeholder_text">
                    ...
                    <input type="text" name="placeholder_text" value="<?php echo esc_attr( $cf->placeholder_text ); ?>"
                           autocomplete="off">
                </div>
			<?php
			endif;
            ...
            <div data-type="single-select" data-required="false" class="wpsc-input-group date-range">
                ...
                <select id="wpsc-date-range" name="date_range">
                    <option <?php selected( $cf->date_range, 'all' ); ?>
                            value="all"><?php esc_attr_e( 'All', 'supportcandy' ); ?></option>
                    <option <?php selected( $cf->date_range, 'future' ); ?>
                            value="future"><?php esc_attr_e( 'Future', 'supportcandy' ); ?></option>
                    <option <?php selected( $cf->date_range, 'past' ); ?>
                            value="past"><?php esc_attr_e( 'Past', 'supportcandy' ); ?></option>
                    <option <?php selected( $cf->date_range, 'range' ); ?>
                            value="range"><?php esc_attr_e( 'Between', 'supportcandy' ); ?></option>
                </select>
                <script>
                    jQuery('#wpsc-date-range').change(function () {
                        if (jQuery(this).val() == 'range') {
            ...
            <div data-type="date-select" data-required="false" class="wpsc-input-group wpsc-date-range-option"
                 style="<?php echo $cf->date_range != 'range' ? 'display:none;' : ''; ?>">
                <div class="label-container">
                    <label for=""><?php esc_attr_e( 'Date in between', 'supportcandy' ); ?></label>
                </div>
                <div class="wpsc-date-range">
                    <div class="wpsc-input-close-group">
                        <input type="text" id="from-date-range" name="from-date-range"
                               value="<?php echo $cf->start_range && is_object( $cf->start_range ) ? esc_attr( $cf->start_range->format( 'Y-m-d' ) ) : ''; ?>"
                               autocomplete="off"/>
                        <span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
                    </div>
                    <div style="margin:5px;"></div>
                    <div class="wpsc-input-close-group">
                        <input type="text" id="to-date-range" name="to-date-range"
                               value="<?php echo $cf->end_range && is_object( $cf->end_range ) ? esc_attr( $cf->end_range->format( 'Y-m-d' ) ) : ''; ?>"
                               autocomplete="off"/>
                        <span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
                    </div>
                </div>
            ...
            <div data-type="date-format" data-required="false" class="wpsc-input-group date-format">
                ...
                <input type="text" name="date_format" value="<?php echo esc_attr( $cf->date_format ); ?>"
                       autocomplete="off"/>
            </div>
			<?php
			...
			if ( in_array( 'tl_width', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="number" data-required="false" class="wpsc-input-group tl_width">
                    ...
                    <input type="number" name="tl_width" value="<?php echo intval( $cf->tl_width ); ?>"
                           autocomplete="off">
                </div>
			<?php
			endif;
			...
			if ( in_array( 'is_personal_info', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="single-select" data-required="false" class="wpsc-input-group is_personal_info">
                    ...
                    <select name="is_personal_info">
                        <option <?php selected( $cf->is_personal_info, '0' ); ?>
                                value="0"><?php esc_attr_e( 'No', 'supportcandy' ); ?></option>
                        <option <?php selected( $cf->is_personal_info, '1' ); ?>
                                value="1"><?php esc_attr_e( 'Yes', 'supportcandy' ); ?></option>
                    </select>
                </div>
			<?php
			...
			if ( in_array( 'allow_my_profile', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="single-select" data-required="false" class="wpsc-input-group allow_my_profile">
                    ...
                    <select name="allow_my_profile">
                        <option <?php selected( $cf->allow_my_profile, 0 ); ?>
                                value="0"><?php esc_attr_e( 'No', 'supportcandy' ); ?></option>
                        <option <?php selected( $cf->allow_my_profile, 1 ); ?>
                                value="1"><?php esc_attr_e( 'Yes', 'supportcandy' ); ?></option>
                    </select>
                </div>
			<?php
			...
			if ( in_array( 'allow_ticket_form', $field_class::$allowed_properties ) ) :
                ...
                <div data-type="single-select" data-required="false" class="wpsc-input-group allow_ticket_form">
                    ...
                    <select name="allow_ticket_form">
                        <option <?php selected( $cf->allow_ticket_form, 0 ); ?>
                                value="0"><?php esc_attr_e( 'No', 'supportcandy' ); ?></option>
                        <option <?php selected( $cf->allow_ticket_form, 1 ); ?>
                                value="1"><?php esc_attr_e( 'Yes', 'supportcandy' ); ?></option>
                    </select>
                </div>
			<?php
		...
		public static function get_ticket_field_val( $cf, $ticket ) {
			...
			return $ticket->{$cf->slug}->format( $format );
		}
		...
		public static function print_tl_ticket_field_val( $cf, $ticket ) {
			echo WPSC_CF_Timer_Templates::ticket_list( $cf, $ticket );
		...
		public static function print_widget_ticket_field_val( $cf, $ticket ) {
			echo WPSC_CF_Timer_Templates::ticket_widget( $cf, $ticket );
			//echo esc_attr( self::get_ticket_field_val( $cf, $ticket ) );
		}
		...
		public static function get_customer_field_val( $cf, $customer ) {
			...
			return $customer->{$cf->slug}->format( $format );
		}
		...
		public static function print_val( $cf, $val ) {
			...
			echo esc_attr( $val->format( $format ) );
		}
